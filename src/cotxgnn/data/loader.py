...
def load_fda_drugs(filepath: Optional[Path] = None) -> pd.DataFrame:
    ...
    Args:
        filepath: JSON 檔案路徑，預設為 data/raw/co_fda_drugs.json

    Returns:
        包含所有藥品的 DataFrame

    Raises:
        FileNotFoundError: 找不到 INVIMA 資料檔案時，提供下載指引
    """
    if filepath is None:
        # loader.py -> data -> cotxgnn -> src -> COTxGNN
        filepath = Path(__file__).parent.parent.parent.parent / "data" / "raw" / "co_fda_drugs.json"

    if not filepath.exists():
        raise FileNotFoundError(
            f"找不到 INVIMA 藥品資料: {filepath}\n"
            f"請先執行以下步驟：\n"
            f"1. 執行: uv run python scripts/process_fda_data.py\n"
            f"   (腳本會自動從 Socrata SODA API 下載資料)"
        )
